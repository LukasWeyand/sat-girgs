...
#include <satgirgs/WeightLayer.h>
...
protected:
    ...
    /**
     * @brief
     *  Sample edges of type 2 between \f$ C_i^A V_j^B \f$
     *  and \f$ V_i^A C_j^B \f$.
    ...
    std::vector<WeightLayer<D>> buildPartition(
        const std::vector<double>& weights, const std::vector<std::vector<double>>& positions, bool clauses);


private:
    EdgeCallback& m_EdgeCallback;    ///< called for every produced edge
    default_random_engine m_gen;     ///< random generator
    const bool m_profile;

    // TODO also split all this into clause and variable version
    ...
    // clauses data
    long long m_m;              ///< number of clauses in the graph
    std::vector<Node<D>>        m_clauses;          ///< nodes ordered by layer first and morton code second
    std::vector<unsigned int>   m_firstC_in_cell;   ///< prefix sums into clauses array
    std::vector<WeightLayer<D>> m_weight_layersC;   ///< provides access to the nodes as described in paper

    // variables data
    long long m_n;              ///< number of variables in the graph
    std::vector<Node<D>>        m_variables;        ///< nodes ordered by layer first and morton code second
    std::vector<unsigned int>   m_firstV_in_cell;   ///< prefix sums into varibales array
    std::vector<WeightLayer<D>> m_weight_layersV;   ///< provides access to the nodes as described in paper
...
template <unsigned int D, typename EdgeCallback>
SpatialTree<D,EdgeCallback> makeSpatialTree(
        const std::vector<double>& weightsC, const std::vector<std::vector<double>>& positionsC,
        const std::vector<double>& weightsV, const std::vector<std::vector<double>>& positionsV,
        double alpha, EdgeCallback& edgeCallback, bool profile = false) {
    return {weightsC, positionsC, weightsV, positionsV, alpha, edgeCallback, profile};
}


} // namespace satgirgs

#include <satgirgs/SpatialTree.inl>
